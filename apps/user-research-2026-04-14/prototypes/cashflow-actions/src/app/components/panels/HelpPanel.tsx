...
export default function HelpPanel() {
  ...
  const renderArticleItem = (article: (typeof helpArticles)[0]) => (
    <li key={article.id}>
      <button
        className="group cursor-pointer flex items-start gap-3 border-b border-border-primary py-2 pl-5 pr-3 text-left hover:bg-background-primary"
        type="button"
      ...
      >
        ...
        <div className="flex size-10 flex-none -rotate-90 items-center justify-center rounded-full group-hover:bg-content-secondary/5">
  ...
  return (
    <>
      ...
      <div className="absolute top-16 w-full left-0 flex items-center border-b border-border-primary gap-3 py-3 pl-5 pr-3">
        <div className="flex w-full items-center justify-between gap-3 rounded-[20px] border border-[#A6A9B0] px-3">
          <Search className="flex-none fill-faint" />
          <input
            className="h-10 w-full border-none bg-transparent text-[15px]/[20px] text-content-secondary outline-none placeholder:text-content-secondary"
      ...
      <div className="fixed bottom-0 right-0 flex w-screen items-center justify-between border-t border-border-primary bg-white p-3 md:w-[400px]">
        <button
          className="flex items-center gap-2 rounded-[3px] border border-transparent px-3 py-[6px] text-[13px]/[20px] font-bold text-brand-primary hover:bg-background-primary"
          type="button"
        >
          Go to Xero central
      ...
      {activeSubPanel === "add-user" && (
        <SlidingSubPanel title="Help">
          <article className="px-5 pb-[64px] pt-5 text-[15px]/[24px]">
          ...
          <div className="fixed bottom-0 right-0 z-20 w-screen bg-white p-3 shadow-[0px_-3px_0px_0px_rgba(0,10,30,0.05),0px_-1px_0px_0px_rgba(0,10,30,0.20)] sm:w-[400px]">
            <button
              className="flex items-center gap-2 rounded-[3px] px-3 py-[6px] text-[13px]/[20px] font-bold text-brand-primary hover:bg-background-primary"
              type="button"
            >
