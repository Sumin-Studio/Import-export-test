...
export function CashflowActionPlanWidget({ className = "" }: { className?: string }) {
  ...
  const chartTitleBold = (pathname?.includes("/purchases-overview/prototype/4") ??
    false)
    ? "Available cash"
    : "Bank balance";
  ...
  const melioHandoffPayload = getMelioHandoffForPlanKey(bankBalanceConfigKey);
  const melioDiyaDemoHandoff = isDiyaDemo && melioHandoffPayload != null;

  const onMelioDiyaDemoAfterApply = useCallback(() => {
    const payload = getMelioHandoffForPlanKey(bankBalanceConfigKey);
    if (!payload) return;
    seedJaxThreadForMelioHandoff(payload, { clearThread, appendMessage });
    openPanel("jax", payload.jaxSubPanel, true);
  }, [bankBalanceConfigKey, appendMessage, clearThread, openPanel]);
  ...
  const panelProps = {
    ...
    melioDiyaDemoHandoff,
    onMelioDiyaDemoAfterApply: melioDiyaDemoHandoff
