...
const CHART_BY_KEY: Record<
  ...
  React.ComponentType<{
    ...
    /** When false, row hover shows the generic linked marker even on days that have a Protect dot. */
...
export function ActionPlanPanel({
  ...
  /** `?scenario=diya-demo`: after Apply plan, hand off to JAX + Melio (prototype). */
  melioDiyaDemoHandoff = false,
  onMelioDiyaDemoAfterApply,
}: {
  ...
  melioDiyaDemoHandoff?: boolean;
  onMelioDiyaDemoAfterApply?: () => void;
}) {
  const isControlled =
  ...
  const { projectedData, currentData, projectedEvents, currentEvents } =
    useMemo(() => {
      ...
      /* Prototype /4: no Protect markers until hover; then show only the hovered link’s marker. */
  ...
  const chartHighlightDayIndex = linking.enabled ? linking.activeDayIndex : null;
  const chartHighlightSeverity: LinkSeverity = linking.enabled
    ? linking.activeSeverity
    : "normal";
  /** When hovering the Protect widget (or linked surface), merge highlight with Protect dots. */
    ...
    >
      ...
      >
        <Chart
          ...
          highlightBandStart={linking.enabled ? linking.activeBandStart : null}
    ...
    billsTable && (
      ...
      >
        <BankBalanceAwaitingBillsAccordion
          ...
          onAfterApplyPlan={
            melioDiyaDemoHandoff ? onMelioDiyaDemoAfterApply : undefined
          }
        />
      </div>
    );
