...
## Iteration 5 / Diya demo — Apply plan → Melio (US bill pay)

Canonical URL: **`/purchases-overview/prototype/4?scenario=diya-demo`** (redirects and in-app links default to this).

- **Make a plan** expands the cash flow plan in the bank widget. **Apply plan** (once the plan is open) opens JAX with **Melio** payment prep — that is the iteration 5 handoff.
- **Opt out of Melio:** `?scenario=in-product` — Apply plan stays in-product only (toast / close modal).

Source: `src/app/lib/cashflowMelioIteration5.ts`. Product limitations are in `docs/cashflow-iteration5-diya-melio-weekly-update.md` (user-research app root).
