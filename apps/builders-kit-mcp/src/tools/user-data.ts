...
// These tools are stubbed — they return example shapes so callers can see
// what a real response would look like. Wire to your actual research data
// source (Dovetail, Notion, Confluence, internal DB) when ready.

export const userDataTools: Tool[] = [
  {
    name: "userdata_query",
    description:
      "[STUB] Query user research data — search studies, sessions, transcripts, and insights. Returns example data; not wired to a real source yet.",
    inputSchema: {
      type: "object",
      properties: {
        ...
        filters: {
          ...
          properties: {
            ...
            date_range: {
              type: "string",
              description: "e.g., 'last 30 days', '2024-01-01 to 2024-03-01'",
            },
            tags: {
              type: "array",
              items: { type: "string" },
  ...
  },
  {
    ...
    description:
      "[STUB] Get categorized insights on a topic from user research. Returns example data; not wired to a real source yet.",
    inputSchema: {
      type: "object",
      properties: {
...
export async function handleUserDataTool(
  name: string,
