...
import { ensureUserExists, markStepIncomplete } from "@/lib/supabase/milestones";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await markStepIncomplete(userId, milestoneKey);

    if (!result.success) {
      return NextResponse.json(
        { error: result.error || "Failed to mark incomplete" },
        { status: 500 }
      );
    }
