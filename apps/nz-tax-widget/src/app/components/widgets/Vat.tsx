...
export function Vat({ className = "", isCustomising = false }: ComponentProps) {
  ...
  return (
    <CustomizationOverlay isCustomising={isCustomising}>
      <div
        className={`relative flex h-auto min-h-[251px] w-[440px] flex-col rounded-xl bg-white transition-all duration-100 ease-in-out lg:h-[251px] lg:min-w-[440px] ${
          isCustomising ? "pointer-events-none" : ""
        } ${className}`}
      >
        <div className="relative flex items-center justify-between pt-5 pr-2 pb-1 pl-6">
        ...
        {(taxContent?.period || taxContent?.ammountDue) && (
          <ul className="border-background-tertiary relative mx-6 mt-3 border-t pt-3">
            <li>
              <button
