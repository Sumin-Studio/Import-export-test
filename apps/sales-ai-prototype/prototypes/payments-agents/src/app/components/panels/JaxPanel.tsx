...
export default function JaxPanel() {
  const { openPanel, openSubPanel, activePanel, activeSubPanel } =
    useNavigation();
  const { getMessages, appendMessage, seedIfEmpty } = useJaxChat();
  ...
  const typewriterDone = lastContent.length > 0 && typedLength >= lastContent.length;
  ...
  }, [messages.length, lastIsAssistant, lastContent.length, isBillDetail]);
  ...
  const handleSend = useCallback(() => {
    ...
    setInputValue("");
  }, [inputValue, isCashflow, isCashflowShortfall, isCashflowCritical, isBillDetail, billEntryKey, billDetailId, isBillsReview, billsEntryKey, messages.length, appendMessage]);
                  ...
                  {isBillDetail ? (
                    <div className="flex flex-wrap gap-2">
                      <Link
                        href={
                          billDetailId
                            ? `/xero-protect/prototype/3/bills/${billDetailId}`
                            : "/xero-protect/prototype/3/bills"
                        }
                      ...
                      <Link
                        href="/xero-protect/prototype/3/bills"
                        className="inline-flex items-center gap-2 rounded-[48px] border border-border-primary bg-white px-4 py-[6px] text-[13px]/[16px] font-bold text-brand-primary transition-colors hover:bg-[#eff1f2]"
                      >
                        Back to bills list
                      </Link>
                    ...
                    <Link
                      href={
                        isBillsReview
                          ? "/xero-protect/prototype/3/bills?from=spotlight"
                          : "/cashflow"
                      }
                    ...
                    >
                      {isBillsReview ? "View bills" : "Go to cashflow manager"}
