...
interface ComponentProps {
  ...
  footerLabel?: string;
  hideOthersRow?: boolean;
  secondColumnLabel?: string;
  showSortArrow?: boolean;
  thirdColumnLabel?: string;
  customRows?: Array<{ initials: string; color: string; textColor?: string; name: string; due: string; overdue: string; isOverdue?: boolean }>;
...
export function ExpensesToReview({
  ...
  hideStats = false,
  firstColumnLabel = "Submitted by",
  squareAvatars = false,
  footerLabel = "View all expenses",
  hideOthersRow = false,
  secondColumnLabel = "Expenses to review",
  showSortArrow = false,
  thirdColumnLabel,
  customRows,
  hideOverflow = false,
            ...
            <thead className="text-[11px]/[16px] text-content-secondary">
              <tr>
                ...
                <th className="py-2 font-normal text-right">
                  <span className="inline-flex items-center gap-0.5">{secondColumnLabel}{showSortArrow && <img src="/icons/Sort-single.svg" alt="" className="inline size-3" aria-hidden />}</span>
                </th>
                <th className="py-2 font-normal text-right">
                  <span>{thirdColumnLabel ?? "Amount"}</span>
                </th>
              </tr>
            </thead>
              ...
              {customRows ? customRows.map((row) => (
                ...
                >
                  <td className="flex gap-3 items-center py-1">
                    <span
                      className={`font-bold size-8 ${squareAvatars ? "rounded-[3px]" : "rounded-full"} flex items-center justify-center text-[13px]/[20px]`}
                      style={{ backgroundColor: row.color, color: row.textColor }}
                    >
                      {row.initials}
                    </span>
                    <span className="text-action-primary">{row.name}</span>
                  </td>
                  <td className="text-right">
                    <span>{row.due}</span>
                  </td>
                  <td className={`text-right ${row.isOverdue ? "text-[#D34246]" : ""}`}>
                ...
                >
                  <td className="flex gap-3 items-center py-1">
                    {expense.initials ? (
                      <span
                        className={`font-bold size-8 ${squareAvatars ? "rounded-[3px]" : "rounded-full"} flex items-center justify-center`}
