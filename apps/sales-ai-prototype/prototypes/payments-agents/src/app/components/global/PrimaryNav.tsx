    ...
    <nav className="relative hidden items-center nav-1049:inline-flex">
      ...
      <div className="flex gap-1">
        ...
        >
          Home
          {pathname === "/" && (
            <span className="absolute left-0 right-0 h-1" style={{ bottom: -12, boxShadow: "inset 0px -4px 0px 0px rgba(255,255,255,0.75)" }} />
          )}
        </Link>
        <NavigationMenu>
          <NavigationMenuList>
            {Object.entries(navigation).map(([key, section]) => {
              const pathSegment = NAV_PATH_MAP[key];
              const isActive = pathSegment ? pathname.includes(pathSegment) : false;
              return (
