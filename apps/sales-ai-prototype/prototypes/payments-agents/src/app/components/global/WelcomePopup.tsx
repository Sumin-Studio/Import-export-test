...
const WELCOME_DISMISSED_COOKIE = "a2a_welcome_dismissed";

function getWelcomeDismissed(): boolean {
  if (typeof document === "undefined") return false;
  return document.cookie.includes(`${WELCOME_DISMISSED_COOKIE}=1`);
}

function setWelcomeDismissed(): void {
  if (typeof document === "undefined") return;
  // Session cookie (no max-age/expires) — cleared when browser/tab session ends
  document.cookie = `${WELCOME_DISMISSED_COOKIE}=1; path=/; SameSite=Lax`;
}
...
export function WelcomePopup() {
  ...
  return (
    <Dialog
      ...
      open={isOpen}
      onClose={handleClose}
      aria-label="Preview the Account to Account NZ experience"
      ...
      <DialogPanel className="w-full max-w-[600px] rounded-2xl overflow-hidden relative">
        ...
        <div className="bg-white py-6 px-8 text-[15px]/[24px]">
          <h2 className="text-[24px]/[133%] font-bold mb-4">
            You’re previewing the Account to Account NZ experience
          </h2>
          <h3 className="mb-3">What’s new?</h3>
          <h4 className="font-bold">High Contrast Banner</h4>
          <p className="mb-4 text-[#404756]">
            Contextual call to action targeting our chosen beta participants to setup this new payment method, when getting paid is top of mind, creating an invoice.
          </p>
          <h4>
            <span className="font-bold">Modal pop-up</span>
          </h4>
          <p className="mb-5 text-[#404756]">
            Based on the user's current status of online payment options, the modal will tailor the CVP accordingly.
          </p>
