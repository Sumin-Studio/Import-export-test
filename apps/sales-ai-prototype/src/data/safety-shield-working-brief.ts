/**
 * Safety Shield Working Brief — source for Confluence sync.
 * Date: 2026-03-10. Update this file to refresh the Confluence page.
 */
export const safetyShieldWorkingBriefMarkdown = `**Overview:** Safety Shield is the bill-risk detection layer for AP: detect bills that deserve a second look, explain why, and route to the safest next action. This brief defines the problem, value, remit, and recommended first slice (duplicates, first-time large, amount spike). Audience: Product, Design, Engineering, Data Science, Leadership.
...
For a full picture of what existing research is relevant, what we do not yet know, and recommended next steps, see the **User research and gaps** page under Safety Shield in this hub.
...
- Exact or near-duplicate bills
- First-time suppliers with unusually large bill values
- Familiar suppliers with materially abnormal amount spikes
...
For a business owner, finance lead, or approver, the problem is simple: they want to move quickly, but they do not want to accidentally approve a duplicate, miss a suspicious change, or send money where it should not go. Safety Shield exists to reduce that cognitive burden by helping Xero surface the bills that matter most.
...
Safety Shield is a shared detection and evidence layer for bill risk. Its job is to answer four questions:
...
**Not Bill Approval** — Bill Approval owns routing, approval policy, review flow, and handoff decisions once a bill needs human action.

**Not Bill Runner** — Bill Runner owns payment timing, liquidity management, and plan optimization.

**Not a full fraud investigation system** — Safety Shield can detect and elevate risk, but it does not replace deeper verification, external remediation, or capital solutions.
...
- **Inline risk surfacing:** Flags in bills lists, approval queues, and payment preparation moments.
- **Evidence-first review:** A bill detail or side panel that explains why the system intervened and what the user should do next.
- **Proactive summaries:** Spotlight-style surfaces that pull the most important issues to the top before the user discovers them manually.

This leads to a simple journey: Monitor → Detect or flag → Provide evidence → Take action → Capture the decision and audit trail.

## Target User

The initial target user is likely a small or medium business customer who is approving or preparing bills for payment. This could include:
...
- **Exact duplicate before approval:** The system identifies a likely duplicate with strong evidence and recommends blocking or removing it from the normal flow.
- **First-time supplier with unusually large bill:** The system cannot prove fraud, but the bill is clearly risky enough to require explicit review.

**Trade-off scenarios**

- **Bank-detail change on a critical supplier:** The system detects a strong fraud signal, but delaying payment introduces operational risk.
- **Recurring supplier with an abnormal amount spike:** The supplier is known, but the bill value is outside the normal range and needs explanation.
- **Many small bills, one bigger pattern:** No single bill is alarming by itself, but a cluster creates a meaningful risk story.

**Fail-gracefully scenarios**

- **Helpful but inconclusive:** The system sees enough to interrupt, but not enough to recommend one confident path.
- **Too late to prevent, but still worth catching:** The system identifies a problem after scheduling or payment, so the value shifts from prevention to containment and follow-up.

## Logic Principles

The first version should likely combine three things:

**Deterministic rules** — Use high-confidence checks where the system can act strongly and explain clearly. Examples: exact or near-duplicate matches, known hard-rule violations, payment-detail changes close to due date.

**Heuristics and risk scoring** — Use lighter anomaly logic where certainty is lower but the signal is still worth surfacing. Examples: first-time suppliers, abnormal supplier amount ranges, unusual clustered spend patterns.

**Human-in-the-loop control** — The early experience should favor interruption, explanation, and traceability over invisible automation. Likely actions include: approve anyway, request review, hold or remove from flow, investigate externally.
...
Future versions may benefit from: OCR-level duplicate detection, cross-customer trust or fraud signals, broader network intelligence.

## What Needs To Be True

The main assumptions to align on are:

- Xero-only data is enough to produce a believable first version for a few high-confidence scenarios.
- Users will trust an interruption if the evidence is simple, specific, and recognizable.
- Safety Shield should remain a distinct detection layer even when embedded inside other experiences.
- The first release should prefer human review over quiet automation.
- The launch surfaces should be narrow enough to feel coherent.
...
A short register to keep design and product risks visible. Reference: [NN/g design risk management](https://www.nngroup.com/articles/design-risk-management/).

| Hazard (before mitigation) | Likelihood | Impact | Mitigation |
| --- | --- | --- | --- |
| Users ignore or distrust interruptions because evidence feels vague or noisy | Occasional | High | Keep first slice narrow (duplicates, first-time large, amount spike); use plain-language explanations and clear evidence. |
| Safety Shield feels like friction rather than protection | Possible | High | Design as exception handling, not every-bill inspection; make "safest next action" obvious and low-effort. |
| Detection runs too late (after payment scheduled) and only surfaces post facto | Possible | Medium | Define which surfaces get real-time vs batch checks; document "fail gracefully" for late detection. |
| Over-automation without human-in-the-loop for first release | Unlikely | High | Default to interrupt + explain + recommend; avoid silent auto-block in v1. |
`;
