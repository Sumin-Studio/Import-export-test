...
/** Highcharts loads only after mount (avoids SSR and `next/dynamic` lazy chunk `.call` errors). */
function JaxAssistantCashflowChart({
  useShortfallChart,
}: {
  useShortfallChart: boolean;
}) {
  ...
  useEffect(() => {
    let cancelled = false;
    setChart(null);
    const p = useShortfallChart
      ? import(
          /* webpackMode: "eager" */ "@/app/components/charts/CashflowShortfallChart"
        )
      : import(
          /* webpackMode: "eager" */ "@/app/components/charts/JaxCashflowThreshold"
        );
    void p.then((m) => {
  ...
  if (!Chart) {
    return (
      <div className="w-full min-w-0" style={{ minHeight: 220 }} aria-hidden />
    );
  }
  return <Chart className="w-full min-w-0" />;
}
