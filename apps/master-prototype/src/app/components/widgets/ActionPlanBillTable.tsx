...
import type { LinkSeverity } from "@/app/contexts/PurchasesInteractiveLinkingContext";
...
export function ActionPlanBillTable({
  ...
  /** When provided with {@link setPlanRowHover}, dims non-hovered rows and feeds chart highlight state. */
      ...
      >
        ...
        <tbody>
          {tableRows.map((row) => {
            ...
            const meta = getLinkMetaForName(row.billName);
            const rowDayIndex = showPlanned
              ? row.plannedDayIndex
              : row.originalDayIndex;
            const rowDimmedByTableHover =
              setPlanRowHover != null &&
              planRowHover != null &&
              planRowHover.billId !== row.id;
            const rowDimmedByProtectLink =
              linking.enabled &&
              linking.activeLinkId != null &&
              meta?.linkId !== linking.activeLinkId;
            const opacityClass =
              setPlanRowHover != null
                ? rowDimmedByTableHover || rowDimmedByProtectLink
                  ? "opacity-45"
                  : "opacity-100"
            ...
            return (
              <tr
                ...
                } ${opacityClass} transition-opacity duration-150`}
                onMouseEnter={() => {
                  setPlanRowHover?.({
                    billId: row.id,
                    dayIndex: rowDayIndex,
                    severity: meta?.severity ?? "normal",
                    snapToProtectDot: meta != null,
                  });
                  linking.setActiveLink(meta?.linkId ?? null, {
                    severity: meta?.severity ?? "normal",
                    dayIndex: meta?.dayIndex ?? rowDayIndex,
                    bandStart: null,
                    bandEnd: null,
                  });
                }}
