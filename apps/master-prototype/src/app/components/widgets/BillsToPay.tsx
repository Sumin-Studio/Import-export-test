...
const loadBillsToPayChart = () =>
  import(/* webpackMode: "eager" */ "../charts/BillsToPay");
...
export function BillsToPay({
  ...
  const Chart = useClientLazyImport<{ className?: string }>(loadBillsToPayChart);
  return (
    <CustomizationOverlay isCustomising={isCustomising}>
      ...
      >
        ...
        {Chart ? (
          <Chart className="mt-4 px-[14px]" />
        ) : (
          <div
            className="mt-4 px-[14px]"
            style={{ minHeight: 280 }}
            aria-hidden
          />
        )}
